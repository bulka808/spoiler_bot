...
@router.message(
    ...
    F.reply_to_message.media_group_id.as_("media_group_id"),
)
async def group_spoiler(message: types.Message, media_group_id: str):
    ...
    [album_builder.add_photo(media=photo, has_spoiler=True) for photo in album.files]

    await message.answer_media_group(media=album_builder.build())
    # TODO доделать нормальное удаление оригинального сообщения с фото
    # (список с id сообщений хранить просто в объекте и по нему удалять)
    await bot.delete_messages(message_ids=album.messages, chat_id=message.chat.id)
...
async def spoiler(message: types.Message, photo: str):
    if not message.reply_to_message.has_media_spoiler:
        ...
        await bot.delete_message(
            message_id=message.reply_to_message.message_id, chat_id=message.chat.id
        )
        await bot.delete_message(message_id=message.message_id, chat_id=message.chat.id)
